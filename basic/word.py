# word.txt 파일을 읽어서 단어 문제를 출제
# word.txt 읽어와서 리스트에 담아서 출력

# word.py 가 실행되고 있는 경로 가져오기
import os
import random
import time
import sqlite3
from datetime import datetime

conn = sqlite3.connect("./basic/test.db", isolation_level=None)

print("현재 폴더", os.getcwd())

words = []
with open("./basic/data/word.txt", "r") as f:
    for word in f:
        words.append(word.strip())

# print(words)

# 타자 게임임
input("게임을 시작할까요? 엔터 입력")
start = time.time()  # 시작시간

# 5 문제 출제
count = 0
loop = 1
while loop <= 5:
    # 리스트 섞어주기
    random.shuffle(words)
    # 임의로 하나 추출
    word1 = random.choice(words)
    # user 는 문제를 타이핑
    print(f"* Question {loop}")
    print(word1)
    player = input()
    # 문제와 user 가 작성한 값이 일치하는지 확인
    if word1.strip() == player.strip():
        print("정답!!")
        # 정답 개수 count 처리
        count += 1
    else:
        print("오답!!!")
    loop += 1

end = time.time()  # 종료 시간

et = format(end - start, ".3f")

print(f"게임시간 : {et}, 정답개수 : {count}")
# 정답 개수 3개 이상이면 합격 / 아니면 불합격
if count >= 3:
    print("합격")
else:
    print("불합격")

# DB 저장
# records 테이블 생성 - 정답개수(integer), 시간기록(et), regdate
cursor = conn.cursor()
sql = "create table if not exists records(correct_answer integer, record text, regdate text)"
cursor.execute(sql)

sql = "insert into records(correct_answer, record, regdate) values(?, ?, ?)"
now = datetime.now()
nowDateTime = now.strftime("%Y-%m-%d %H:%M:%S")
cursor.execute(sql, (count, et, nowDateTime))

conn.close()
